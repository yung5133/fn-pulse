...
| 情形 | 推进 | 例子 |
| --- | --- | --- |
| **同一功能的迭代 / 修补**（绝大多数情况） | 修订号 | `0.6.0` → `0.6.1` |
| 新增功能或新模块 | 次版本号 | `0.6.1` → `0.7.0` |
| 不兼容变更 | 主版本号 | `0.7.0` → `1.0.0` |

用脚本推进，避免手改出错：

```bash
python tools/bump_version.py patch    # 0.6.0 -> 0.6.1
python tools/bump_version.py minor    # 0.6.1 -> 0.7.0
```

> **版本号只向前走。** 已推送并产出镜像的版本不可回改，否则使用者无法判断
> 哪个更新；写错方向时只能继续递增来修正。
> CI 会在 `app/` 有改动而 `VERSION` 未变时直接失败，提醒补 bump。
