...
每次推送到 `main` 都会自动构建并打上**明确的版本 tag**（`latest` 之外），可直接锁定与回滚：

```bash
# 查看本次构建的版本号（也写在 Actions 运行摘要里）
docker pull ghcr.io/yung5133/fn-pulse:0.3.12
docker compose up -d
```

版本规则：`<主版本>.<构建序号>`，主版本取自仓库根目录的 `VERSION` 文件（唯一来源），
构建序号取自 Actions 的 run number，两者拼接后还会作为构建参数注入容器 ——
所以**界面上显示的版本号与镜像 tag 永远一致**，不会出现"不知道跑的是哪个版本"。

其他标签规则：

| tag | 何时产生 |
| --- | --- |
| `0.3.<N>` | 每次推送 main（N 为递增构建序号） |
| `latest` | 每次推送 main |
| `v1.2.3` 等语义化版本 | 推送 `v*` 格式的 git tag 时 |
| `sha-<短哈希>` | 每次构建 |

想发布正式版本：改一下 `VERSION` 文件（如 `0.3` → `0.4`）再推送，或直接推 `v0.4.0` tag。
